...
static uint8_t s_map_all_zero = 0u;
...
static uint8_t s_blast_mismatch_count = 0u;
static uint8_t s_blast_event_index = 0u;
static uint8_t s_entry_started = 0u;
...
static uint8_t mission_map_structurally_valid(
    const char map[SOKOBAN_MAP_HEIGHT][SOKOBAN_MAP_STRIDE], uint8_t row, uint8_t col)
{
    ...
    for(y = 0u; y < SOKOBAN_MAP_HEIGHT; y++)
    {
        for(x = 0u; x < SOKOBAN_MAP_WIDTH; x++)
        {
            ...
            else if((map[y][x] == '.') || (map[y][x] == 'G') || (map[y][x] == '+')) goal_count++;
        }
    }
    return (uint8_t)((goal_count > 0u) && (normal_box_count == goal_count) &&
                     (bomb_count <= SOKOBAN_MAX_BOMBS) &&
                     ((uint8_t)(normal_box_count + bomb_count) <= SOKOBAN_MAX_BOXES));
}
    ...
    for(row = 0u; row < SOKOBAN_MAP_HEIGHT; row++)
    {
        for(col = 0u; col < SOKOBAN_MAP_WIDTH; col++)
        {
            ...
            if((ch == '$') || (ch == '*'))
            {
                ...
                if(ch == '*') s_labels.box_labels[s_labels.box_count] = SOKOBAN_BOMB_LABEL;
                s_labels.box_count++;
            }
            else if((ch == '.') || (ch == 'G') || (ch == '+'))
            {
...
static void mission_read_frame(void)
{
    ...
    uint8_t all_zero;
    s_frame_updated = 0u;
    if(!camera_sokoban_copy_latest_frame(s_current_map, &row, &col, &all_zero, &sequence)) return;
    if(sequence == s_last_frame_sequence) return;
    s_last_frame_sequence = sequence;
    s_frame_updated = 1u;
    s_player_row = row;
    s_player_col = col;
    s_map_all_zero = all_zero;
    if(!s_have_valid_map && mission_map_structurally_valid(s_current_map, row, col))
    {
        s_have_valid_map = 1u;
        s_timer_running = 1u;
    }
...
static uint8_t mission_start_segment(const sokoban_solution_t *source, uint16_t begin, uint16_t end)
{
    uint16_t idx;
    mission_solution_reset(&s_segment_solution);
    if((begin > end) || (end > source->move_count)) return 0u;
    for(idx = begin; idx < end; idx++) s_segment_solution.move_seq[idx - begin] = source->move_seq[idx];
...
static void mission_enter_fault(sokoban_status_t status)
{
...
static void mission_begin_inspection(void)
{
    ...
    s_last_status = sokoban_plan_inspection(s_initial_map, s_heading, &s_inspection_plan);
    if(s_last_status != SOKOBAN_STATUS_OK)
    {
        mission_enter_fault(s_last_status);
        return;
    }
    if(!mission_initialize_labels())
    {
        mission_enter_fault(SOKOBAN_STATUS_INVALID_MAP);
        return;
    }
    s_inspection_event = 0u;
    s_inspection_move_index = 0u;
    s_segment_running = 0u;
    s_state = MISSION_EXECUTE_INSPECTION;
}
...
static uint8_t mission_blast_map_matches(uint8_t blast_index)
{
    ...
    for(row = 0u; row < SOKOBAN_MAP_HEIGHT; row++)
    {
        for(col = 0u; col < SOKOBAN_MAP_WIDTH; col++)
        {
            ...
            if(boundary && (s_initial_map[row][col] == '#') && (s_current_map[row][col] != '#')) return 0u;
            if(!boundary && (s_initial_map[row][col] == '#'))
            {
                ...
                uint8_t dr = (uint8_t)(row > blast_row ? row - blast_row : blast_row - row);
                uint8_t dc = (uint8_t)(col > blast_col ? col - blast_col : blast_col - col);
                if((dr <= 1u) && (dc <= 1u) && (s_current_map[row][col] == '#')) return 0u;
            }
        }
    }
    return 1u;
...
void mission_controller_process(void)
{
    ...
    switch(s_state)
    {
        case MISSION_ENTER_START_ZONE:
            if(s_entry_started == 0u)
            {
                mission_solution_reset(&s_segment_solution);
                s_segment_solution.solved = 1u;
                s_segment_solution.move_count = 1u;
                s_segment_solution.move_seq[0] = 'R';
                s_segment_solution.move_seq[1] = '\0';
                if(path_executor_start(&s_segment_solution)) s_entry_started = 1u;
            }
            else if(path_executor_is_done())
            {
                s_entry_distance = path_executor_get_last_step_distance();
                if(s_have_valid_map) mission_begin_inspection();
                else s_state = MISSION_WAIT_VALID_MAP;
            }
            break;
        ...
        case MISSION_EXECUTE_INSPECTION:
            ...
            {
                s_state = MISSION_PLAN_LABELED_PUSH;
                break;
            }
            if(s_segment_running == 0u)
            {
                uint16_t end = s_inspection_plan.events[s_inspection_event].move_index;
                if(end == s_inspection_move_index) s_state = MISSION_WAIT_TURN;
                else if(mission_start_segment(&s_inspection_plan.route, s_inspection_move_index, end)) s_segment_running = 1u;
                else mission_enter_fault(SOKOBAN_STATUS_INVALID_MAP);
            }
            else if(path_executor_is_done())
            {
                s_segment_running = 0u;
                s_inspection_move_index = s_inspection_plan.events[s_inspection_event].move_index;
                s_state = MISSION_WAIT_TURN;
            }
            break;

        case MISSION_WAIT_TURN:
        {
            const sokoban_inspection_event_t *event = &s_inspection_plan.events[s_inspection_event];
        ...
        case MISSION_WAIT_SCAN_RESULT:
        {
            ...
            label = (event->object_type == (uint8_t)SOKOBAN_OBJECT_BOX) ?
                            s_labels.box_labels[event->object_index] : s_labels.goal_labels[event->object_index];
            if(label != 0u)
            {
                s_inspection_event++;
                s_state = MISSION_EXECUTE_INSPECTION;
        ...
        case MISSION_PLAN_LABELED_PUSH:
            s_last_status = sokoban_solve_labeled(s_initial_map,
                                                  s_inspection_plan.final_row,
                                                  s_inspection_plan.final_col,
                                                  &s_labels, &s_task_solution);
            if(s_last_status != SOKOBAN_STATUS_OK) mission_enter_fault(s_last_status);
            else
            {
                uint16_t end;
                s_blast_event_index = 0u;
        ...
        case MISSION_EXECUTE_PUSH:
            ...
            {
                s_segment_running = 0u;
                if(s_blast_event_index < s_task_solution.blast_count) s_state = MISSION_WAIT_BLAST_MAP;
                else s_state = MISSION_WAIT_MAP_CLEAR;
            }
            break;

        case MISSION_WAIT_BLAST_MAP:
            if(s_frame_updated && mission_blast_map_matches(s_blast_event_index))
            {
                uint16_t begin = (uint16_t)(s_task_solution.blast_move_indices[s_blast_event_index] + 1u);
                uint16_t end;
                s_blast_mismatch_count = 0u;
                s_blast_event_index++;
                end = (s_blast_event_index < s_task_solution.blast_count) ?
                      (uint16_t)(s_task_solution.blast_move_indices[s_blast_event_index] + 1u) :
                      s_task_solution.move_count;
                if(begin == s_task_solution.move_count) s_state = MISSION_WAIT_MAP_CLEAR;
            ...
            else if(s_frame_updated)
            {
                s_blast_mismatch_count++;
                if(s_blast_mismatch_count >= 3u) mission_enter_fault(SOKOBAN_STATUS_INVALID_MAP);
            }
            break;
        ...
        case MISSION_WAIT_MAP_CLEAR:
            if(s_frame_updated)
            {
                if(s_map_all_zero)
                {
                    if(s_clear_frame_count < 3u) s_clear_frame_count++;
                    if(s_clear_frame_count >= 3u) s_state = MISSION_PLAN_RETURN_ENTRY;
                }
                else s_clear_frame_count = 0u;
            }
            break;

        case MISSION_PLAN_RETURN_ENTRY:
            s_last_status = sokoban_plan_walk(s_current_map, s_player_row, s_player_col,
                                              s_entry_row, s_entry_col, &s_task_solution);
            if(s_last_status != SOKOBAN_STATUS_OK) mission_enter_fault(s_last_status);
            else if(s_task_solution.move_count == 0u) s_state = MISSION_RETURN_PHYSICAL_START;
            else if(path_executor_start(&s_task_solution)) s_state = MISSION_EXECUTE_RETURN_ENTRY;
            else mission_enter_fault(SOKOBAN_STATUS_INVALID_MAP);
            break;

        case MISSION_EXECUTE_RETURN_ENTRY:
            if(path_executor_is_done()) s_state = MISSION_RETURN_PHYSICAL_START;
            break;

        case MISSION_RETURN_PHYSICAL_START:
            if(s_segment_running == 0u)
            {
                mission_solution_reset(&s_segment_solution);
                s_segment_solution.solved = 1u;
...
uint8_t mission_controller_set_label(sokoban_object_type_t type, uint8_t object_index, uint8_t label)
{
    ...
    {
        s_labels.box_labels[object_index] = label;
        return 1u;
    }
    if((type == SOKOBAN_OBJECT_GOAL) && (object_index < s_labels.goal_count))
    {
        s_labels.goal_labels[object_index] = label;
        return 1u;
    }
    return 0u;
}
...
{
    if(((s_state == MISSION_WAIT_TURN) || (s_state == MISSION_WAIT_SCAN_RESULT)) &&
...
void mission_label_process(void)
{
       ...
       LABEL_REPLY_TIMEOUT_10MS)
    {
        ...
        else
        {
            /*
             * 三次都没有得到合法结果。
             * mission_enter_fault()在同一个C文件中，
             * 因此这里可以直接调用。
             *
             * 当前没有专门的通信超时错误码，
             * 暂时只能借用INVALID_LABELS。
             */
            mission_enter_fault(
